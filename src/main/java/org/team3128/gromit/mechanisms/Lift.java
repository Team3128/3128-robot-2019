...
/**
 * Control system for the two-stage, continuous elevator-style lift mechanism.
 * The {@link FourBar} is mounted to the first stage of the list.
 * 
...
public class Lift {
	public enum LiftHeightState {
		...
		INTAKE_FLOOR_CARGO(8 * Length.in), VISION(35 * Length.in),

		SHIP_CARGO(30 * Length.in),

		LOW_CARGO(16 * Length.in), MID_CARGO(49 * Length.in), TOP_CARGO(80 * Length.in),

		HATCH_INTAKE(21.5 * Length.in), HATCH_PULL_UP(28.5 * Length.in),

		LOW_HATCH(26.5 * Length.in), MID_HATCH(33.5 * Length.in), TOP_HATCH(67.5 * Length.in); // 69.5
	...
	public enum LiftControlMode {
		PERCENT(2, "Percent Output"), POSITION_UP(0, "Position (Up)"), POSITION_DOWN(1, "Position (Down)");
	 ...
	 * 
	 * MAX ENCODER POSITION = 51,745 native units MAX HEIGHT = 78.5 inches
	 */
	...
	public static void initialize(LiftHeightState state, TalonSRX liftMotor, DigitalInput limitSwitch,
	...
	private Lift(LiftHeightState state, TalonSRX liftMotor, DigitalInput limitSwitch, int limitSwitchLocation,
			int liftMaxVelocity) {
	...
	/*
	 * public class CmdZero extends Command { public CmdZero() { super(0.2); }
	 * 
	 * @Override protected void initialize() { override = true; powerControl(-0.2);
	 * }
	 * 
	 * @Override protected boolean isFinished() { return getLimitSwitch() ||
	 * isTimedOut(); }
	 * 
	 * @Override protected void end() { override = false;
	 * liftMotor.setSelectedSensorPosition(limitSwitchLocation, 0,
	...
	public class CmdHeightControl extends Command {
		...
		protected void end() {
			powerControl(0);
			cmdControlled = false;
			Log.debug("CmdSetLiftPosition",
					"Lift at desired height of " + (heightState.targetHeight / Length.in) + " inches.");
		}
		...
		protected void interrupted() {
			...
			cmdControlled = false;
			Log.debug("CmdSetLiftPosition",
					"Interrupted. Final height = " + (getCurrentHeight() / Length.in) + " inches.");
		}
		...
		protected boolean isFinished() {
			...
			if (plateauCount > ERROR_PLATEAU_COUNT)
				return true;
