 ...
 * 
 * When instantiated, it sets the distance from the absolute angle. So, between
 * 0 and 1 rotations. When reset, the distance goes to zero.
 * 
...
public class CTREMagneticEncoder extends QuadratureEncoder {
	...
	/**
	 * 
	 * @param dataAPort               DIO port with the "A" data line plugged in
	 *                                (pin 7 on the encoder)
	 * @param dataBPort               DIO port with the "B" data line plugged in to
	 *                                it (pin 5 on the encoder)
	 * @param pwmPort                 DIO port connected to pin 9 on the encoder,
	 *                                the PWM pin
	 * 
	 * @param measureStartingPosition If true, use the absolute PWM signal to figure
	 *                                out the absolute position of the encoder when
	 *                                the class is constructed. Of course, this can
	 *                                only be between 0 and 360 degrees, so if your
	 *                                encoder rotates multiple times, this is pretty
	 *                                much useless.
	 * 
	 * @param startingOffset          Degree offset TO add to the initial position
	 *                                measurement. In other word, offset between the
	 *                                zero point of your encoder and the zero point
	 *                                of your mechanism. If measureStartingPosition
	 *                                is false this does nothing.
	 * 
	 *                                The PWM signal is used to get absolute
	 *                                position, while the quadrature inputs are used
	 *                                for relative position and velocity.
	 * 
	 * 
	...
	public CTREMagneticEncoder(int dataAPort, int dataBPort, int pwmPort, boolean inverted,
			boolean measureStartingPosition, double startingOffset) {
	...
	/**
	 * Returns the absolute angle of the encoder from the PWM signal. Each full
	 * rotation starts this angle back at 0.
	 * 
	 * @return
	 */
