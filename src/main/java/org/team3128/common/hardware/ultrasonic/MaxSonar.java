...
/**
 * Class to read the Maxbotics MaxSonar sensor plugged into the RoboRIO RS232
 * port. It is accurate to a few millimeters and has a max range of 500 cm.
 * 
...
public class MaxSonar implements IUltrasonic {
	...
	public static enum Resolution {
		...
		INCH(Length.in, 3, 255), CM(Length.cm, 3, 765), // cm-resolution sensors
	...
	/**
	 * 
	 * @param rangingPinDIONumber The DIO pin that the ranging pin on the ultrasonic
	 *                            is connected to.
	 * @param resolution          The resolution from the of the sensor as described
	 *                            by the MaxBotix website. The MB1013 is mm, and the
	 *                            MB1010 is inch.
	 * @param portToUse           which serial port to use. Note that if using the
	 *                            onboard serial port, you will need to disable the
	 *                            console out in the RoboRIO webpage.
	 */
		...
		try {
			...
			if (ex.getMessage().indexOf("Resource Busy") > -1) {
				throw new RuntimeException(
						"Could not construct the serial port as it is in use.  Have you disabled Console Out on the RoboRIO?",
						ex);
			} else
				throw ex;
		}
	...
	private Pair<Boolean, Integer> getDistanceFromResponse(String response) {
		...
		if (response == null || response.length() < 2) {
			Log.recoverable("MaxSonar",
					"Got bad response from sensor:\"" + (response == null ? "<null>" : response) + "\"!");
		...
		try {
			...
			// Log.debug("MaxSonar", "Measured distance as " + (distance *
			// sensorResolution.conversionFactor) + " cm");

			return new Pair<Boolean, Integer>(Boolean.TRUE, distance);
		} catch (NumberFormatException ex) {
			ex.printStackTrace();
			Log.recoverable("MaxSonar",
					"Got bad response from sensor, coudn't convert to an integer: " + numberPart + "!");
			return new Pair<Boolean, Integer>(Boolean.FALSE, 0);
		}
	...
	@Override
	/**
	 * According to the datasheet, pinging the sensor manually reduces its accuracy
	 */
