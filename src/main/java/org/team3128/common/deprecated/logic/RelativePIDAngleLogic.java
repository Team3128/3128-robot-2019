package org.team3128.common.deprecated.logic;

import org.team3128.common.hardware.encoder.distance.IDistanceEncoder;
import org.team3128.common.deprecated.limiter.MotorLogic;
 ...
 * 
 * This class uses a relative encoder, like an optical or magnetic one that
 * counts revolutions, as opposed to an absolute one like a potentiometer.
 * 
...
public class RelativePIDAngleLogic extends MotorLogic {
     ...
     * @param stopWhenDone whether to stop controlling the motor when it's reached
     *                     its target
     * @param encoder
     * @param stopSwitch   a switch that, when <strong>low</strong>, signifies that
     *                     the motor has reached its stop point. If it is provided,
     *                     calling clearSpeedControlRun() will automatically,
     *                     asynchronously run the motor until it hits this point. If
     *                     it is null, calling clearSpeedControlRun() will just set
     *                     the current position of the motor to the zero point.
     */
    ...
    public double speedControlStep(double dt) {
        ...
        if (isAutoResetting) {
            ...
            if (_log) {
                // Log.debug(TAG, "target: " + targetAngle + " current: " + angle + " error: " +
                // error + " output: " + output);
            }
     ...
     * 
     * This controller maintains an internal angle offset, so getting this info from
     * the encoder may not work.
     * 
     * @return
     */
