...
public class AbsolutePIDAngleLogic extends MotorLogic {
     ...
     * @param encoder
     */
    public AbsolutePIDAngleLogic(double kP, double kI, double kD, double threshold, boolean stopWhenDone,
            IDistanceEncoder encoder, boolean log) {
        _refreshTime = 10;
    ...
    public double speedControlStep(double dt) {
        ...
        if (_log) {
            // Log.debug("PIDAngleTarget", "target: " + targetAngle + " current: " + angle +
