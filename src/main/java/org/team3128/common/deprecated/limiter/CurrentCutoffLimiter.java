...
/**
 * Motor control which moves the motor in a certain direction until its current
 * spikes, which (hopefully) means that it has hit the end of its travel.
 */
...
public class CurrentCutoffLimiter extends Limiter {
     ...
     * @param currentThreshold measured amperage where motor is considered stalled.
     *                         You have to measure this for each individual motor.
     * @param refreshMillis
     */
    public CurrentCutoffLimiter(PowerDistributionPanel panel, int motorChannel, double currentThreshold,
            int refreshMillis) {
        _panel = panel;
