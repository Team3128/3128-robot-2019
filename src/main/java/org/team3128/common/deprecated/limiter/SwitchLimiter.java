 ...
 * 
 * Note that this class assumes that running the motor backwards moves toward
 * the minimum endstop.
 * 
 * @author Jamie
 *
...
public class SwitchLimiter extends Limiter {
	...
	/**
	 * 
	 * @param minSwitch limit switch which can be reached by setting the motor to
	 *                  reverse
	 * @param maxSwitch limit switch which can be reached by setting the motor to go
	 *                  forward
	...
	public boolean canMove(double power) {
		...
		// Log.debug("LimitSwitchEndstop", "hitMinStop: " + hitMinStop + " hitMaxStop: "
		// + hitMaxStop);
