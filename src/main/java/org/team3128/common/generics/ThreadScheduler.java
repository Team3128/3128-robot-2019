...
public class ThreadScheduler implements Runnable {
			...
			if (!paused) {
				synchronized (this) {
					for (Schedule schedule : schedules) {
						/*
						 * if (schedule.removeNextFrame) { safeRemove(schedule); } else {
						 */
	...
	/*
	 * public void remove(Schedule schedule) { schedule.removeNextFrame = true; }
	 * 
	 * private void safeRemove(Schedule schedule) { schedules.remove(schedule); }
	 */
