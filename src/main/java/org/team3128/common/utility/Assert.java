...
public class Assert {
	 ...
	 * 
	 * Try to use one of the more specific functions below if possible so that a
	 * more detailed error message can be printed.
	 * 
	 * @param condition
	 */
	...
	public static void equals(Object obj1, Object obj2) {
		if (!obj1.equals(obj2)) {
			assertFail(String.format("%s is not equal to %s!", obj1 == null ? "null" : obj1.toString(),
	...
	public static <T extends Comparable<T>> void greaterThan(T object, T lowerBound) {
		if (object.compareTo(lowerBound) <= 0) {
			assertFail(String.format("%s is not greater than %s!", object == null ? "null" : object.toString(),
	...
	public static <T extends Comparable<T>> void lessThan(T object, T upperBound) {
		if (object.compareTo(upperBound) >= 0) {
			assertFail(String.format("%s is not less than %s!", object == null ? "null" : object.toString(),
	...
	private static void assertFail(String message) {
		...
		for (int index = 2; index < stackTrace.length; ++index) {
			failureMessage.append(String.format("at %s.%s() (%s:%d)\n", stackTrace[index].getClassName(),
