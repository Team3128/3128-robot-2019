...
import org.team3128.common.utility.datatypes.RandomAccessBuffer;
...
public class PIDCalculator {
	...
	// buffer containing the time duration that the feedback was a value, and the
	// value
	...
	/**
	 * @param constants
	 * @param izone     the number of past measurements to keep when calculating the
	 *                  I term
	 * @param threshold the "acceptable" amount of error inside which the value is
	 *                  considered close enough and the threshold count will be
	 *                  incremented
	 */
	...
	private double calculateIntegral() {
		...
		for (int index = 1; index <= pastValues.getLastIndex(); ++index) {
			integral += ((pastValues.get(index - 1).right + pastValues.get(index).right))
					* pastValues.get(index - 1).left;
		}
		integral /= 2;
	...
	public double update(double value) {
		// Cap the delta time to prevent it going through the roof when the robot is
		// disabled
		...
		if (log) {
			Log.debug("PIDCalculator: " + logName,
					String.format("Input: %.04f, Error: %.04f, output: %.04f, integral: %.04f, derivative: %.04f",
							value, error, output, calculateIntegral(), derivative));
		}
	...
	/**
	 * Get the number of update() calls in a row ending with the most recent one
	 * that the error has been inside the threshold
	 * 
	 * @return
	 */
