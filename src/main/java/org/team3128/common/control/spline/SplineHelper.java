...
public final class SplineHelper {
   ...
   * user is free to set the direction of the start and end point. The directions
   * for the middle waypoints are determined automatically to ensure continuous
   ...
   *         provided waypoints.
   */
  @SuppressWarnings({ "LocalVariableName", "PMD.ExcessiveMethodLength", "PMD.AvoidInstantiatingObjectsInLoops" })
  public static CubicHermiteSpline[] getCubicSplinesFromWaypoints(Pose2D start, Translation2D[] waypoints, Pose2D end) {
    ...
    if (waypoints.length == 0) {
      scalar = 1.2 * start.getTranslation().getDistanceTo(end.getTranslation());
    } else {
      scalar = 1.2 * start.getTranslation().getDistanceTo(waypoints[0]);
    }

    double[] xInitialControlVector = { start.getTranslation().getX(), scalar * start.getRotation().cos() };
    double[] yInitialControlVector = { start.getTranslation().getY(), scalar * start.getRotation().sin() };

    // This just makes the splines look better.
    if (waypoints.length != 0) {
      scalar = 1.2 * end.getTranslation().getDistanceTo(waypoints[waypoints.length - 1]);
    }

    double[] xFinalControlVector = { end.getTranslation().getX(), scalar * end.getRotation().cos() };
    double[] yFinalControlVector = { end.getTranslation().getY(), scalar * end.getRotation().sin() };
    ...
    if (waypoints.length > 1) {
      ...
      dx[dx.length - 1] = 3 * (newWaypts[newWaypts.length - 1].getX() - newWaypts[newWaypts.length - 3].getX())
          - xFinalControlVector[1];
      dy[dy.length - 1] = 3 * (newWaypts[newWaypts.length - 1].getY() - newWaypts[newWaypts.length - 3].getY())
          - yFinalControlVector[1];
      ...
      for (int i = 0; i < newFx.length - 1; i++) {
        splines[i] = new CubicHermiteSpline(new double[] { newWaypts[i].getX(), newFx[i] },
            new double[] { newWaypts[i + 1].getX(), newFx[i + 1] }, new double[] { newWaypts[i].getY(), newFy[i] },
            new double[] { newWaypts[i + 1].getY(), newFy[i + 1] });
      }
    } else if (waypoints.length == 1) {
      final var xDeriv = (3 * (end.getTranslation().getX() - start.getTranslation().getX()) - xFinalControlVector[1]
          - xInitialControlVector[1]) / 4.0;
      final var yDeriv = (3 * (end.getTranslation().getY() - start.getTranslation().getY()) - yFinalControlVector[1]
          - yInitialControlVector[1]) / 4.0;

      double[] midXControlVector = { waypoints[0].getX(), xDeriv };
      double[] midYControlVector = { waypoints[0].getX(), yDeriv };

      splines[0] = new CubicHermiteSpline(xInitialControlVector, midXControlVector, yInitialControlVector,
          midYControlVector);
      splines[1] = new CubicHermiteSpline(midXControlVector, xFinalControlVector, midYControlVector,
          yFinalControlVector);
    } else {
      splines[0] = new CubicHermiteSpline(xInitialControlVector, xFinalControlVector, yInitialControlVector,
          yFinalControlVector);
    }
    return splines;
  }
  ...
  /**
   * Returns a set of quintic splines corresponding to the provided waypoints. The
   * user is free to set the direction of all waypoints. Continuous curvature is
   * guaranteed throughout the path.
   *
  ...
  public static QuinticHermiteSpline[] getQuinticSplinesFromWaypoints(Pose2D[] waypoints) {
    ...
    for (int i = 0; i < waypoints.length - 1; i++) {
      ...
      // This just makes the splines look better.
      final var scalar = 1.2 * p0.getTranslation().getDistanceTo(p1.getTranslation());

      double[] xInitialControlVector = { p0.getTranslation().getX(), scalar * p0.getRotation().cos(), 0.0 };
      double[] xFinalControlVector = { p1.getTranslation().getX(), scalar * p1.getRotation().cos(), 0.0 };
      double[] yInitialControlVector = { p0.getTranslation().getY(), scalar * p0.getRotation().sin(), 0.0 };
      double[] yFinalControlVector = { p1.getTranslation().getY(), scalar * p1.getRotation().sin(), 0.0 };

      splines[i] = new QuinticHermiteSpline(xInitialControlVector, xFinalControlVector, yInitialControlVector,
          yFinalControlVector);
    }
    return splines;
  }
