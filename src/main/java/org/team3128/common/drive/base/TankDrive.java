package org.team3128.common.drive.base;

import static java.lang.Math.abs;

import org.team3128.common.autonomous.AutoUtils;
import org.team3128.common.hardware.encoder.both.QuadratureEncoder;
import org.team3128.common.deprecated.limiter.MotorGroup;
import org.team3128.common.utility.Log;
import org.team3128.common.utility.RobotMath;
import org.team3128.common.utility.VelocityPID;
...
public class TankDrive implements ITankDrive {
	 ...
	 */
	public TankDrive(MotorGroup leftMotors, MotorGroup rightMotors, QuadratureEncoder encLeft,
			QuadratureEncoder encRight, double wheelCircumfrence, double gearRatio, double wheelBase, double track) {
	...
	public void arcadeDrive(double joyX, double joyY, double throttle, boolean fullSpeed) {
		...
		// Log.debug("TankDrive", "x1: " + joyX + " throttle: " + throttle + " spdR: " +
		// spdR + " spdL: " + spdL);
	...
	public enum MoveEndMode {
		BOTH, // ends when both sides have reached their targets. Keeps moving both sides
				// until then. Make sure both encoders are working!
		EITHER, // Stops both sides when either side has reached its target.
		PER_SIDE // Stops each side when it reaches its target. The command ends when both sides
					// hit their targets.
	}
	...
	public class CmdMoveDistance extends Command {
		...
		protected boolean isFinished() {
			...
			// Log.debug("CmdMoveDistance", "Left dst: " + encLeft.getDistanceInDegrees() +
			// ", Right dst: " + encRight.getDistanceInDegrees());
			switch (endMode) {
			case BOTH:
	 ...
	 * 
	 * Runs the opposite motors from the direction provided, so turning LEFT would
	 ...
	 * ----------------------------------------------------- UNTESTED CODE! This
	 * class has never been tried on an actual robot. It may be non or partially
	 * functional. Do not make any assumptions as to its behavior! And don't blink.
	 * Not even for a second. -----------------------------------------------------
	 */

	/**
	 * Command to stop the robot. Useful on bigger, heavier robots (like TheClawwww)
	...
	public class CmdInPlaceTurn extends CmdMoveDistance {
		...
		public CmdInPlaceTurn(float degs, double motorPower, int msec, Direction dir) {
			// the encoder counts are an in-depth calculation, so we don't set them until
	...
	public class CmdMoveForward extends CmdMoveDistance {
		...
		public void end() {
			super.end();
			Log.debug("CmdMoveForward", "The right side went " + ((encRight.getAngle() * 100.0) / encRight.getAngle())
					+ "% of the left side");
		}
	}
	...
	public class CmdMoveStraightForward extends Command {
		...
		/**
		 * @param d    how far to move. Accepts negative values.
		 * @param kP   The konstant of proportion. Scales how the feedback affects the
		 *             wheel speeds.
		 * @param msec How long the move should take. If set to 0, do not time the move
		 */
		...
		protected void end() {
			...
			Log.debug("CmdMoveStraightForward", "The right side went at "
					+ ((encRight.getAngle() * 100.0) / encRight.getAngle()) + "% of the left side");
		}
