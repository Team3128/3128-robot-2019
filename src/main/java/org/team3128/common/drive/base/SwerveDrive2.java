...
import org.team3128.common.deprecated.logic.RelativePIDAngleLogic;
...
public class SwerveDrive2 {
	...
	private static class SwerveModule {
		...
		/**
		 * Sets the angle and power of the swerve module. May also reverse the wheel to
		 * minimise movement time.
		 * 
		 * @param targetAngle
		 * @param direction
		 */
	...
	/**
	 * Construct a swerve drive with no gyro. Driver-oriented control will be
	 * unavailable.
	 */
	public SwerveDrive2() {
	 ...
	 * 
	 * You will still have to enable driver-oriented control by calling the
	 * function, though.
	 * 
	 * @param gyro
	 */
	...
	/**
	 * Set whether driver-oriented control is enabled. If there is no gyro, it will
	 * always be disabled.
	 * 
	 * @param enable
	 */
	...
	final static private double IS_TURNING_THRESHOLD = .2; // controller input threshold above which the robot is
															// considered to be turning.
	 ...
	 * 
	 * @param turnMotor          the MotorGroup of the motor controlling the
	 *                           rotation of the wheel. Rotating the motor forwards
	 *                           should rotate the wheel counterclockwise.
	 * @param driveMotor         the MotorGroup of the motor moving the wheel.
	 * @param encoder            the distance encoder (e.g. a quadrature encoder)
	 *                           hooked up to the turning motor. COunterclockwise
	 ...
	 * @param d
	 */
	public void addModule(MotorGroup turnMotor, MotorGroup driveMotor, IDistanceEncoder encoder,
			DigitalInput homingSwitch, double homingSwitchOffset, double angleOnWheelbase, double homingOffset,
			double p, double i, double d) {
		SwerveModule newModule = new SwerveModule();

		newModule.driveMotor = driveMotor;

		newModule.controller = new RelativePIDAngleLogic(p, i, d, 1, false, encoder, homingSwitch, homingSwitchOffset,
				true);
